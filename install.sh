...
if [ ! -f "${FORTRAN_ML_ROOT}/BUILD/lib/libmlCoupling.so" ]; then
    ...
    cd ${FORTRAN_ML_ROOT}
    mkdir -p BUILD && cd BUILD
    cmake .. -Dh5fortran_ROOT=${FORTRAN_ML_ROOT}/extern/h5fortran/BUILD/INSTALL -DTensorflow_DIR=${FORTRAN_ML_ROOT}/extern/tensorflow/ -DTensorflow_Python_DIR=${FORTRAN_ML_VENV}/lib/python3.11/site-packages/tensorflow -DWITH_AIX=ON -DWITH_PHYDLL=ON -DWITH_NCSA=ON
    cmake --build . -j && cmake --install .
